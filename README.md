...
2.  The Pi should immediately turn on. It will then boot into the Circle environment, which takes about 7 seconds on a Pi Zero W 2 and an A1-class card. If the target computer is set up to use USB devices, it should quickly see the Pi as an optical drive.
    ...
    - **Windows 98 QuickInstall users — analog CD audio is fixed.** On QuickInstall v1.0.0 and later, analog CD audio (the built-in Windows CD Player) did not work: those versions replaced Windows' native USB storage stack with the SweetLow USB 2.0 stack, and the PLAY AUDIO command never reached the drive. It affected any USB CD-ROM drive, not just USBODE. **SweetLow** (also known as [LordOfMice](https://github.com/LordOfMice)) diagnosed it and wrote `USBCDHLP.VXD`, a small IOSUBSYS helper that restores the ATAPI command path for USB CD-ROM drives while leaving ordinary USB storage alone. It is confirmed working with USBODE. Download it from:

      `http://sweetlow.orgfree.com/download/usbcdhlp.zip`

      **Copy that address and paste it into your browser's address bar rather than clicking it.** The site can reject requests that arrive from another page, in which case clicking returns an error instead of the download.

      To install, copy `USBCDHLP.VXD` into `C:\WINDOWS\SYSTEM\IOSUBSYS\` and reboot. Leave QuickInstall's patched `USBNTMAP.SYS` exactly as it is — the helper is designed to sit alongside it, not to replace it. If you used an earlier build of the helper, open Device Manager once and remove any leftover CD-ROM entry belonging to a device that is not a CD drive, then reconnect it; that entry is cached in the registry and will otherwise persist. Should you ever want to disable the helper, delete or move the file — renaming it has no effect, because Windows loads every driver in that folder regardless of its name.

      Our thanks to SweetLow for tracking this down, writing the fix, and releasing the binaries into the public domain. The original report is [win98-quickinstall#151](https://github.com/oerg866/win98-quickinstall/issues/151). Data discs and digital audio playback (e.g. Winamp) were never affected.
